...
console.log(fenomenler[0].profile); // dot notation
console.log(fenomenler[0]["profile"]); // bracket notation
...
function indekseGoreFenomen(celebrities, index) {
  return `${index}. indekste bulunan fenomen: ${celebrities[index].profile}`;
}

// console.log(indekseGoreFenomen(fenomenler, 3), "indekseGoreFenomen");
...
function profilListesi(celebrities) {
  let newList = [...celebrities];
  for (let i = 0; i < celebrities.length; i++) {
    newList[i] = celebrities[i].profile;
  }
  return newList;
}

// console.log(profilListesi(fenomenler), "profilListesi");
...
function fenomenSil(celebrities, index) {
  let newList = [...celebrities];
...
function fenomenEkle(celebrities, number, profile, followers, posts, platform) {
  let newList = [...celebrities];
  const newCelebrity = {
    number,
    profile,
...
function enFenomenler(celebrities) {
  let filteredCelebrities = [];
  for (let i = 0; i < celebrities.length; i++) {
    if (celebrities[i].followers > 100000000) {
      filteredCelebrities.push(celebrities[i].profile);
    }
  }
  return filteredCelebrities;
}

// console.log(enFenomenler(fenomenler), "enFenomenler");
...
function fenomenGonderimSayisi(celebrities, profile) {
  for (let i = 0; i < celebrities.length; i++) {
    if (celebrities[i].profile === profile) {
      return celebrities[i].posts;
    }
  }
  // // Alternative solution
  // const celebrity = celebrities.find(
  //   (celebrity) => celebrity.profile === profile
  // );
  // return celebrity.posts;
}

// console.log(fenomenGonderimSayisi(fenomenler, "Will Smith"), "fenomenGonderimSayisi");
...
function platformaGoreCokGonderiYapanFenomen(celebrities, platform) {
  let maxPosts = 0;
  let profile = "";
  for (let i = 0; i < celebrities.length; i++) {
    if (celebrities[i].platform === platform) {
      if (celebrities[i].posts > maxPosts && celebrities[i].posts !== "NA") {
        maxPosts = celebrities[i].posts;
        profile = celebrities[i].profile;
      }
    }
  }
  return profile;
...
// console.log(platformaGoreCokGonderiYapanFenomen(fenomenler, "TikTok"), "platformaGoreCokGonderiYapanFenomen");
